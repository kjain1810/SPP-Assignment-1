...
void mat_mul(int idx_arr1, int idx_arr2, int idx_ret, int direction)
{
    ...
    for (int c = 0; c < k; c++)
        for (int a = 0; a < n; a++)
        {
            long long x = 0;
            for (int b = 0; b < m; b++)
                x += arr[idx_arr1 * stride + a * m + b] * arr[idx_arr2 * stride + b + c * m];
            if (direction == 0)
                arr[idx_ret * stride + a * k + c] += x;
            else
                arr[idx_ret * stride + a + n * c] += x;
        }
}
    ...
    if (i == j)
    {
        if (direction == 1)
        {
            for (int a = 0; a < x[i]; a++)
                ...
                {
                    long long xx = arr[i * stride + a * y[i] + b];
                    arr[i * stride + a * y[i] + b] = arr[i * stride + a + b * x[i]];
                    arr[i * stride + a + b * x[i]] = xx;
                }
        }
        return i;
    }
